...
class PotatoDiseaseClassifier():
    ...
    def run_classifier_infernce(self):
        try:
            self.model = self.load_classifier_model(model_path=f"{os.getcwd()}{os.sep}models{os.sep}potato_disease_classifier.h5")
            self.label = self.load_classifier_labels(labels_path=f"{os.getcwd()}{os.sep}models{os.sep}potato_disease_classifier_labels.txt")
            # cv2_img = cv2.cvtColor(self.input_img, cv2.COLOR_RGB2BGR)
            new_img = cv2.resize(self.input_img, (256, 256))
