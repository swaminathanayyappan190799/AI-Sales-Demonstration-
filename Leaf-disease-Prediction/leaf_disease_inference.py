...
class LeafDiseasePredictor():
    ...
    def run_classifier_infernce(self):
        try:
            self.model = self.load_classifier_model(model_path=f"{os.getcwd()}{os.sep}models{os.sep}leaf_disease_classifier.h5")
            self.label = self.load_classifier_labels(labels_path=f"{os.getcwd()}{os.sep}models{os.sep}leaf_disease_labels.txt")
